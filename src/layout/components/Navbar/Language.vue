<template>
  <div v-if="navbar.showLanguage">
    <el-dropdown>
      <div class="flex max-w-[200px] cursor-pointer items-center justify-center p-3 duration-300 hover:bg-[#f8f8f9]">
        <svg t="1682504965962" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2481" width="16" height="16">
          <path
      ...
      <template #dropdown>
        <div class="py-2" :class="{ isActive: i18n.global.locale.value == 'zh-CN' }">
          <el-dropdown-item @click="updateI18n('zh-CN')">简体中文</el-dropdown-item>
        </div>
        <div class="py-2" :class="{ isActive: i18n.global.locale.value == 'en-US' }">
          <el-dropdown-item @click="updateI18n('en-US')">English</el-dropdown-item>
        </div>
      </template>
    </el-dropdown>
  </div>
...
import i18n, { modifyHtmlTitle } from '@/i18n'
import { useSettingsStore } from '@/store/modules/settings'

const route = useRoute()
const { updateLanguage, navbar } = useSettingsStore()

const updateI18n = (languageKey: 'zh-CN' | 'en-US') => {
  updateLanguage(languageKey)
  modifyHtmlTitle(route.meta.title)
}
</script>
